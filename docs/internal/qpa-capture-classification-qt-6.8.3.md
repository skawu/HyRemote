...
`src/runtime/src/widgets/widget_target.cpp` creates owned CPU `RemoteFrame` storage, wraps it in a `QImage` with `QImage::Format_RGBA8888_Premultiplied`, sets the target DPR, clears it, and calls:
...
`src/runtime/src/quick/quick_target.cpp` resolves `QQuickWindow::contentItem()` and calls:
