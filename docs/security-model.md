...
The three profiles are not interchangeable and the Runtime keeps them apart: `Insecure` is unauthenticated and unencrypted, `Authenticated` is RFB VNC authentication over an unencrypted stream, and `AuthenticatedEncrypted` is VeNCrypt 0.2 + X509Vnc 261 + TLS >= 1.2 with VNC Authentication inside the tunnel on Qt's **OpenSSL** backend only. The encrypted profile fails with a deterministic `SecurityUnavailable` or `InvalidConfiguration` **before any listener is created** - no target, transport or socket is composed and nothing is left listening - when the OpenSSL backend, its plugin or its runtime is unavailable, or when the certificate and private key do not parse and match. It never falls back to `Authenticated`, to `SecurityType None`, to Schannel or to plaintext. Certificate and key are validated before the listener exists, so descriptor validity, material validity and backend capability stay three separate facts.
