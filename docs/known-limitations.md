...
`Authenticated` therefore means exactly this today: RFB VNC authentication over an **unencrypted** stream, and nothing more.

`AuthenticatedEncrypted` is **unavailable in V0.1** and fails closed rather than degrading. Selecting it makes `start()` fail deterministically with `SecurityUnavailable` **before any listener is created** - no target, no transport and no socket are composed, so no port is left listening - and there is no fallback to `Authenticated` or to `SecurityType None`, even when the certificate and private-key descriptor is present and syntactically valid. A descriptor proves what was configured, not what the transport can negotiate. The final VeNCrypt 0.2 + X509Vnc (subtype 261) + TLS >= 1.2 backend is V1.0.0.0 work under #143, and non-loopback production policy remains V0.2 work under #170/#174.
...
`src/cpp/tests/test_listener_address_matrix.cpp`, which chooses every port through the operating system, so the
