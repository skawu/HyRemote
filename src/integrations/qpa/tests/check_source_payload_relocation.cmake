...
# The loader reports the runtime path it actually used, and for a relocated module that is the anchor form written into
# the plugin's RUNPATH (`$ORIGIN/../../lib/...`), so the reported path legitimately contains `..` segments. Comparing
# it directly against a resolved path is wrong, and file(REAL_PATH) is not dependable either: it keeps the input
# unchanged - CMake only warns - when the path does not resolve, which turns a spelling difference into a false
# failure. Normalize textually, so the assertion stays about *which* file the loader selected, and assert existence
# separately so a truncated or genuinely wrong capture is still reported precisely.
