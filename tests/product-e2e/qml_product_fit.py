...
def main() -> int:
    ...
    try:
        wait_until(process, lines, lambda: any(f"READY {port}" in line for line in lines),
                   "qml-basic did not reach Running", timeout=8)
        ...
        print("PASS: qml-basic import HyRemote -> viewer -> client-count 0/1/reconnect -> pointer/key/text -> stop")
        return 0
    finally:
